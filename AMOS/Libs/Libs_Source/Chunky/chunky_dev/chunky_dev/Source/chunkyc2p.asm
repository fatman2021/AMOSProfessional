...
;
; A few changes in 1998 by Stephan Rupprecht <stephan.rupprecht@gmx.de>
; converted absolute addressing to relative one using the stack
; inlined make_table, removed 68000/010 support
;
...
andval_lead		EQU	46
andval_trail		EQU	47
andval_single		EQU	48
dummy			EQU	49
insertpointer		EQU	50
insertpointerlong	EQU	54
planepointerspointer	EQU	58
sizeof_buffer		EQU	62
;------------------------------------------------------------------------------
	Section	code,CODE
...
ChunkyToPlanarAsm:
	...
	lea.l	-sizeof_buffer(sp),sp

	lea.l	made_table(pc),a2
	tst.b	(a2)			* need only make bitspreadtable once
	bne.s	table_made
...
table_made
	...
	* the mask to be used
	...
	move.b	#$ff,d1
	lsl.b	d0,d1
	move.b	d1,andval_lead(sp)

leadno	move.w	d0,leadingbits(sp)

	move.w	width(a0),d0
	cmp.w	leadingbits(sp),d0
	bhs.s	notsingle
	...
	* a single byte
	...
	addq	#1,d0
	moveq	#8,d2
	sub.w	leadingbits(sp),d2
	lsr.b	d2,d1
	not.b	d1
	move.b	d1,andval_single(sp)
	clr.w	trailingbits(sp)
	bra.s	single
notsingle
	clr.b	andval_single(sp)

	* find number of not bytealigned pixels at right side of frame and
	* the mask to be used
	sub.w	leadingbits(sp),d0
	move.w	d0,d1
	andi.w	#$fff8,d1
	sub.w	d1,d0
	move.w	d0,trailingbits(sp)
	move.b	#$ff,d1
	lsr.b	d0,d1
	move.b	d1,andval_trail(sp)
single
	* initialize variables to be used (modulo, bytealigned width, etc)
	...
	move.b	Depth(a1),d0
	move.w	d0,depth(sp)
	move.w	height(a0),Height(sp)
	move.w	width(a0),Width(sp)
	move.w	leadingbits(sp),d1
	sub.w	d1,Width(sp)
	move.w	trailingbits(sp),d1
	sub.w	d1,Width(sp)
	move.w	Width(sp),GWidth(sp)
	move.w	BytesPerRow(a1),Modulo(sp)
	tst.b	andval_single(sp)
	bne.s	singlebyte
	move.w	Width(sp),d0
	lsr.w	#3,d0
	sub.w	d0,Modulo(sp)
singlebyte
	tst.w	leadingbits(sp)
	beq.s	nolead
	sub.w	#1,Modulo(sp)
nolead	tst.w	trailingbits(sp)
	beq.s	notrail
	sub.w	#1,Modulo(sp)
notrail
...
line0
	...
	* (depending on bitmap depth)
	move.l	a1,a6
	add.l	#Planes,a6
	lea	insertpointerslong(pc),a4
	lea	insertpointers(pc),a5
	moveq	#0,d7
	move.w	depth(sp),d7
	lsl.w	#2,d7
	add.l	d7,a6
	move.l	a6,planepointerspointer(sp)
	subq	#4,d7
	add.l	d7,a4
	add.l	d7,a5
	move.l	(a4),insertpointerlong(sp)
	move.l	(a5),insertpointer(sp)
...
convert8bytes
	...
	* spread bits to all registers *
	...
	beq.s	its8
	move.l	a2,d7
its8	move.l	planepointerspointer(sp),a6
	move.l	insertpointer(sp),a5
	jsr	(a5)
...
rowdone
	...
	* of the frame?
	tst.w	trailingbits(sp)
	beq.b	trailno
	bsr.w	inserttrailingbits

trailno	move.w	GWidth(sp),Width(sp)
	add.w	Modulo(sp),a2
	subq	#1,Height(sp)
	tst.w	Height(sp)
	bne.w	convertlines
...
eachlong
	...
	* insert the converted c->p data into the bitmap
	...
	beq.b	its8x
	move.l	a2,d7
its8x	move.l	planepointerspointer+4(sp),a6
	move.l	insertpointerlong+4(sp),a5
	jsr	(a5)
	rts
;------------------------------------------------------------------------------
insertleadingbits:
...
convertleadingbits
	...
	* spread bits to all registers *
	...
	move.w	depth+4+4*4(sp),a0
	addq	#Planes,a1

	* read and mix in the data from the bitmap with the converted data
planel0	move.w	d7,a5
	move.l	(a1),a3
	add.l	a2,a3
	move.b	(a3),d7
	and.b	andval_lead+4+4*4(sp),d7
	or.b	d7,d0
	move.w	a5,d7
	move.w	d0,a5
...
doneplanes
	...
	beq.s	its8l
	move.l	a2,d7
its8l	move.l	planepointerspointer+4(sp),a6
	move.l	insertpointer+4(sp),a5
	jsr	(a5)
	addq	#1,a2
	rts
;------------------------------------------------------------------------------
...
converttrailingbits
	...
	* read and mix in the data from the bitmap with the converted data
planet0	move.w	d7,a5
	move.l	(a1),a3
	add.l	a2,a3
	move.b	(a3),d7
	and.b	andval_trail+4+4*4(sp),d7
	or.b	d7,d0
	move.w	a5,d7
	move.w	d0,a5
...
planet1	addq	#4,a1
	...
	add.l	a2,a3
	move.b	(a3),d0
	and.b	andval_trail+4+4*4(sp),d0
	or.b	d0,d1
...
done_planes
	...
	beq.s	its8t
	move.l	a2,d7
its8t	move.l	planepointerspointer+4(sp),a6
	move.l	insertpointer+4(sp),a5
	jsr	(a5)
	addq	#1,a2
	rts
...
convertsinglebytebits
	...
	* read and mix in the data from the bitmap with the converted data
planes0	move.w	d7,a5
	move.l	(a1),a3
	add.l	a2,a3
	move.b	(a3),d7
	and.b	andval_single+4+4*4(sp),d7
	or.b	d7,d0
	move.w	a5,d7
	move.w	d0,a5
...
planes1	addq	#4,a1
	...
	add.l	a2,a3
	move.b	(a3),d0
	and.b	andval_single+4+4*4(sp),d0
	or.b	d0,d1
...
done__planes
	...
	beq.s	its8s
	move.l	a2,d7
its8s	move.l	planepointerspointer+4(sp),a6
	move.l	insertpointer+4(sp),a5
	jsr	(a5)
	addq	#1,a2
	rts
