					...
					switch(TagItem->ti_Tag)
					{
						...
						case LAST_CursorPosition:
							...
							*Value = 0;
							...
							{
								if(Node->Type == TEXTEDIT_KIND && Gadget != NULL)
								{
									GetAttr(STRINGA_BufferPos,Gadget,(ULONG *)&Value);
								}
							}
			...
			switch(Node->Type)
			{
				...
				#ifdef DO_TEXTEDIT_KIND
				{
					case TEXTEDIT_KIND:
					{
						...
						if(Gadget != NULL && GetAttr(STRINGA_Buffer,Gadget,&Storage))
							return((LONG)Storage);
						else
							return((LONG)Node->Special.TextEdit.String);
					}
				}
				#endif /* DO_TEXTEDIT_KIND */
