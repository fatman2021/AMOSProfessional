		...
		switch(Node->Type)
		{
			...
			#ifdef DO_TEXTEDIT_KIND
			{
				case TEXTEDIT_KIND:
					...
					if(Node->Special.TextEdit.TextAttr != NULL)
					{
						struct TextFont * font;

						font = LTP_OpenFont(Node->Special.TextEdit.TextAttr);
						if(font == NULL)
						{
							Handle->Failed = TRUE;
							return;
						...
						if(Node->Special.TextEdit.Font != NULL)
							CloseFont(Node->Special.TextEdit.Font);

						Node->Special.TextEdit.Font = font;
					}

					if(Node->Special.TextEdit.Font != NULL)
					{
						Node->Width		= 6 + Node->Chars * Node->Special.TextEdit.Font->tf_XSize + 6;
						Node->Height	= 3 + Node->Lines * Node->Special.TextEdit.Font->tf_YSize + 3;
					}
					else
					{
						Node->Width		= 6 + Node->Chars * Handle->GlyphWidth + 6;
