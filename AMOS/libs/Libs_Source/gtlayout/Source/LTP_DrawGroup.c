...
{
	LTP_DrawGroupMember(Handle,Group,ANY_OBJECT);
}

BOOL
LTP_DrawGroupMember(LayoutHandle *Handle,ObjectNode *Group,LONG id)
{
	BOOL found = FALSE;
	ObjectNode *Node;
	ULONG page;
	...
	SCANPAGE(Group,Node,page)
	{
		...
		switch(Node->Type)
		{
			...
			case GROUP_KIND:
				...
				LTP_DrawGroupMember(Handle,Node,id);
			...
			#ifdef DO_BOOPSI_KIND
			{
				case BOOPSI_KIND:
					...
					if(id == Node->ID)
						RefreshGList(Node->Host,Handle->Window,NULL,1);
