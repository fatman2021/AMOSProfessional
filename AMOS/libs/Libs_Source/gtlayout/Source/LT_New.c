				...
				while(TagItem = NextTagItem(&TempList))
				{
					...
					switch(TagItem->ti_Tag)
					{
						...
						case LA_LastGadget:
							...
							#ifdef DO_TEXTEDIT_KIND
							{
								if(Type == TEXTEDIT_KIND)
									Node->Special.TextEdit.LastGadget = ti_Data;
							}
							#endif /* DO_TEXTEDIT_KIND */
						...
						case LALV_TextAttr:
							...
							{
								if(Type == TEXTEDIT_KIND)
									Node->Special.TextEdit.TextAttr = (struct TextAttr *)ti_Data;
							}
							#endif /* DO_TEXTEDIT_KIND */
						...
						case GTST_String:
							...
							{
								if(Type == TEXTEDIT_KIND)
									Node->Special.TextEdit.String = (STRPTR)ti_Data;
							}
							#endif /* DO_TEXTEDIT_KIND */
						...
						case LAST_Activate:
							...
							{
								if(Type == TEXTEDIT_KIND)
									Node->Special.TextEdit.Activate = ti_Data;
							}
							#endif /* DO_TEXTEDIT_KIND */
...
*
*	    TEXTEDIT_KIND (multiline string gadget):
*
*	        (requires gtlayout.library V46 or higher)
*
*	        LATE_String (STRPTR) - Pointer to the string to be
*	            edited.
*
*	        LATE_CursorPosition (ULONG) - Index number of the
*	            character the cursor should be positioned on.
*	            For example, in the string "abcd", the index
*	            number 0 would put the cursor on the "a" and
*	            the number 3 would put it on the "d".
*
*	        LATE_Chars (LONG) - Width of the text editing area
*	            in characters.
*
*	        LATE_Lines (LONG) - Height of the text editing area
*	            in lines.
*
*	        LATE_TextAttr (struct TextAttr *) - You can specify the
*	            font to be used for the editing area.
*	            The TextAttr (or TTextAttr) you provide must be ready
*	            to go so the layout code can open the font later.
*
*	            To get the current system default font, which is
