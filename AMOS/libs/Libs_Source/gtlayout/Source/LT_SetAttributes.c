...
*	TEXTEDIT_KIND:
*
*	    LATE_String (STRPTR) - The string to put into the editing
*	        area.
*
*	    LATE_CursorPosition (LONG) - Index number of the character
*	        to put the cursor on. Pass -1 to put it to the end
*	        of the string.
*
*	All objects:
*
			...
			switch(Node->Type)
			{
				...
				#ifdef DO_TEXTEDIT_KIND
				{
					case TEXTEDIT_KIND:
						...
						if((ThisTag = FindTagItem(GA_Disabled,TagList)) != NULL)
							Node->Disabled = ThisTag->ti_Data;
						...
						if(ThisTag = FindTagItem(GTST_String,TagList))
						{
							if(Node->Host != NULL)
							{
								SetGadgetAttrs(Node->Host,handle->Window,NULL,
									STRINGA_Buffer,ThisTag->ti_Data,
								TAG_DONE);
							}
							else
							{
								STRPTR string = (STRPTR)ThisTag->ti_Data;
	
								if(Node->Special.TextEdit.Backup != NULL)
									LTP_Free(handle,Node->Special.TextEdit.Backup,strlen(Node->Special.TextEdit.Backup)+1);
	
								Node->Special.TextEdit.Backup = (STRPTR)LTP_Alloc(handle,strlen(string)+1);
								if(Node->Special.TextEdit.Backup != NULL)
								{
									strcpy(Node->Special.TextEdit.Backup,string);
		
									Node->Special.TextEdit.String = Node->Special.TextEdit.Backup;
								}
							}
						}
